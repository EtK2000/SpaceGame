...
public class GUIText {
	private String text;
	private float x, y;
	private char twoPixelLetters[] = { '.', ',' };
	private char sevenPixelLeters[] = { 'i', 'l', 't' };

	public GUIText(String text, float x, float y) {
		this.text = text;
		this.x = x - text.length() * 4;// x is center, char width is {8, 7, 2}
		this.y = DisplayHandler.height - y + 4;// y is center, char height is 8
		for (int i = 0; i < text.length(); i++) {
			for (int j = 0; j < twoPixelLetters.length; j++) {
				if (text.charAt(i) == twoPixelLetters[j]) {
					x -= 1;
					break;
				}
			}
			for (int j = 0; j < sevenPixelLeters.length; j++) {
				if (text.charAt(i) == sevenPixelLeters[j]) {
					x -= 0.5;
					break;
				}
			}
